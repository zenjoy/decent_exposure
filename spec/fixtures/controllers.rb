...
class BirdController < ActionController::Base
  ...
  def show
    render (Rails::VERSION::MAJOR > 4 ? :plain : :text) => "Foo"
  end

  def index
    self.bird = Parrot.new(:beak => "custom")
    render (Rails::VERSION::MAJOR > 4 ? :plain : :text) => "index"
  end

  def new
    render (Rails::VERSION::MAJOR > 4 ? :plain : :text) => "new"
  end
end
...
class StrongParametersController < ActionController::Base
  ...
  def show
    render (Rails::VERSION::MAJOR > 4 ? :plain : :text) => "show"
  end
...
module ::Namespace
  ...
  class ModelController < ActionController::Base
    include Rails.application.routes.url_helpers
    expose(:model)
    def show; render (Rails::VERSION::MAJOR > 4 ? :plain : :text) => ""; end
  end
end
